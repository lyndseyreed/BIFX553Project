...
def run_hmmsearch(query_sequence, hmm_profile):
    '''
    ...
    cmd = ["hmmsearch", "--max", "--tblout", "temp_results.out", hmm_profile, query_file]
...
def categorize_sequences(input_fasta, kinase_profiles, phosphatase_profile):
    ...
    with open(input_fasta, "r") as f:
        for record in SeqIO.parse(f, "fasta"):
            ...
            phosphatase_hits = run_hmmsearch(record, phosphatase_profile)
            if phosphatase_hits:
                is_phosphatase = True
            ...
            if not is_phosphatase:
                for kinase_name, kinase_profile in kinase_profiles.items():
                    kinase_hits = run_hmmsearch(record, kinase_profile)
                    if kinase_hits:
    ...
    with open("kinase_predict.txt", "w") as kinase_file:
        for seq_id, results in kinase_results.items():
            for result in results:
                kinase_file.write(f"{seq_id}: Predicted Kinase - {result[0]}\n")
    with open("phosphatase_predict.txt", "w") as phosphatase_file:
